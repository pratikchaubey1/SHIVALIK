          ...
          <div className="p-6 border-b">
            <div className="flex justify-between items-center mb-4">
              ...
              <div className="flex gap-2">
                <button onClick={() => pagination.hasPrev && fetchOrders(page - 1)} disabled={!pagination.hasPrev} className="px-3 py-1 border rounded disabled:opacity-50">Prev</button>
                <button onClick={() => pagination.hasNext && fetchOrders(page + 1)} disabled={!pagination.hasNext} className="px-3 py-1 border rounded disabled:opacity-50">Next</button>
              </div>
            </div>
            <select 
              value={statusFilter}
          ...
          <div className="bg-white rounded-lg max-w-4xl w-full max-h-[90vh] overflow-y-auto">
            <div className="p-6 border-b">
              <div className="flex justify-between items-center">
                <h3 className="text-lg font-semibold">Order Details - #{selectedOrder._id.slice(-6)}</h3>
                <button 
              ...
              <div className="grid md:grid-cols-2 gap-6">
                <div>
                  ...
                  <div className="space-y-1 text-sm">
                    <div><strong>Order ID:</strong> #{selectedOrder._id.slice(-6)}</div>
                    <div><strong>Date:</strong> {new Date(selectedOrder.orderDate).toLocaleString()}</div>
                    <div><strong>Total:</strong> ₹{selectedOrder.total}</div>
                    <div><strong>Payment ID:</strong> {selectedOrder.payment?.razorpay_payment_id}</div>
                  </div>
                </div>
                    ...
                    <tbody className="divide-y">
                      {selectedOrder.items.map((item, index) => (
                        <tr key={index}>
                          ...
                          <td className="px-4 py-2 text-right text-sm font-medium">₹{(parseFloat(item.price) * item.quantity).toFixed(2)}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
                  <div className="px-4 py-3 bg-gray-50 text-right">
                    <div className="text-sm">Subtotal: ₹{selectedOrder.subtotal}</div>
                    <div className="text-sm">Tax: ₹{selectedOrder.tax}</div>
                    <div className="text-sm">Shipping: ₹{selectedOrder.shipping}</div>
                    <div className="text-lg font-bold mt-1">Total: ₹{selectedOrder.total}</div>
                  </div>
                </div>
              </div>
